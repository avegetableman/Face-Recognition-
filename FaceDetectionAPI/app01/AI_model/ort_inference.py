...
path = r'G:\6\AI_code\cv\retinanet\FaceDetectionAPI\app01\AI_model\detect.onnx'
session = ort.InferenceSession(path)
input_name = session.get_inputs()[0].name
output_names = [session.get_outputs()[0].name, session.get_outputs()[1].name]
image = cv2.imread(r'G:\6\AI_code\cv\retinanet\FaceDetectionAPI\app01\AI_model\hzy.jpg').astype(np.float32) / 255.0
data0 = crop(image)
data0 = resize(data0)
...
cv2.imwrite('face_onnx.jpg', new_data0*255)
