...
def normalize_image(img, mean, std):
    ...
    参数：
    - img (np.ndarray): 输入图像，形状为 (H, W, C)，像素值范围在 [0, 255]。
    - mean (tuple): 每个通道的均值，长度为 3。
    - std (tuple): 每个通道的标准差，长度为 3。
    
    返回：
    - np.ndarray: 归一化后的图像，形状与输入图像相同。
    """
    # 确保输入图像是 float32 类型，并将像素值范围调整到 [0, 1]
    img = img.astype(np.float32) / 255.0
    
    # 将 mean 和 std 转换为 NumPy 数组
